...
main()
{
  ...
  show_time_only=$(get_tmux_option "@dracula-show-time-only" false)
  ...
  for plugin in "${plugins[@]}"; do
    ...
    if [ $plugin = "time" ]; then
      ...
      if $show_time_only; then # only time
        script="%R ${timezone} "
      elif $show_day_month && $show_military ; then # military time and dd/mm
